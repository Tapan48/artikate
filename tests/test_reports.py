...
def test_summary_four_numbers_one_query(
    api, employee_factory, checkout_factory, django_assert_num_queries
):
    ...
    assert response.data == {
        ...
        "currently_held_count": 2,
        "currently_overdue_count": 1,
        "mean_hold_duration_days": 3.25,
    }
...
def test_summary_empty_and_unknown(api, employee_factory):
    ...
    response = api.get(f"/api/v1/employees/{employee.employee_code}/summary/")
    assert response.data == {
        "lifetime_checkout_count": 0,
        "currently_held_count": 0,
...
def test_asset_create_validation_filter_search_and_pagination(api, asset_factory):
    ...
    assert (
        api.post(
            "/api/v1/assets/", {**data, "asset_tag": "BAD", "status": "CHECKED_OUT"}
        ).status_code
        == 400
    )
    assert (
        api.post("/api/v1/assets/", {**data, "asset_tag": "BAD", "category": "UNKNOWN"}).status_code
        == 400
    )
    for _ in range(24):
    ...
    assert (
        api.get("/api/v1/assets/?status=AVAILABLE&category=CAMERA&search=blue").data["count"] == 1
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("post", "/api/v1/checkouts/1/return/"),
        ("get", "/api/v1/employees/EMP-1/summary/"),
