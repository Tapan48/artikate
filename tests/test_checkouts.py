...
def payload(asset, employee, due_at=None):
    return {"asset_tag": asset.asset_tag, "employee_code": employee.employee_code, "due_at": (due_at or timezone.now() + timedelta(days=1)).isoformat()}
...
def test_checkout_and_return(api, asset_factory, employee_factory):
    ...
    assert detail["current_holder"] == {"employee_code": employee.employee_code, "full_name": employee.full_name}
...
def test_return_needing_maintenance(api, checkout_factory):
    ...
    assert api.post(f"/api/v1/checkouts/{checkout.pk}/return/", {"needs_maintenance": True}).status_code == 200
...
def test_unavailable_asset(api, asset_factory, employee_factory, status):
    assert api.post("/api/v1/checkouts/", payload(asset_factory(status=status), employee_factory())).status_code == 409
    assert CheckOut.objects.count() == 0


def test_inactive_employee(api, asset_factory, employee_factory):
    assert api.post("/api/v1/checkouts/", payload(asset_factory(), employee_factory(is_active=False))).status_code == 400
...
@pytest.mark.parametrize("offset,expected", [(timedelta(0), 400), (timedelta(seconds=-1), 400), (timedelta(seconds=1), 201), (timedelta(days=30), 201), (timedelta(days=30, microseconds=1), 400)])
def test_due_date_boundaries(api, asset_factory, employee_factory, offset, expected):
    ...
    with patch("lending.services.timezone.now", return_value=now):
        assert api.post("/api/v1/checkouts/", payload(asset_factory(), employee_factory(), now + offset)).status_code == expected


@pytest.mark.parametrize("field,value,status", [("asset_tag", "missing", 404), ("employee_code", "missing", 404), ("due_at", "nonsense", 400), ("due_at", None, 400)])
...
def test_checkout_rollback(asset_factory, employee_factory):
    ...
    with patch.object(Asset, "save", side_effect=RuntimeError("simulated write failure")):
        with pytest.raises(RuntimeError):
            check_out_asset(asset_tag=asset.asset_tag, employee_code=employee.employee_code, due_at=timezone.now() + timedelta(days=1))
    asset.refresh_from_db()
    assert asset.status == Asset.Status.AVAILABLE
    assert CheckOut.objects.count() == 0
...
def test_database_disallows_two_open_loans(checkout_factory, employee_factory):
    ...
    with pytest.raises(IntegrityError), transaction.atomic():
        CheckOut.objects.create(asset=checkout.asset, employee=employee_factory(), due_at=timezone.now() + timedelta(days=1))
...
def test_constraint_violation_becomes_conflict(api, checkout_factory):
    ...
    assert api.post("/api/v1/checkouts/", payload(checkout.asset, checkout.employee)).status_code == 409
