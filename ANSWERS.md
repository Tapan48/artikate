...
**Pull requests:** GitHub Actions would install locked dependencies on Python 3.12 with PostgreSQL 15 and Redis 7 services, run Ruff checks, Django checks, `makemigrations --check --dry-run`, and the PostgreSQL suite including real concurrency tests. Apply migrations to an empty test database. For schema changes, also rehearse the upgrade from the previous release using representative fixtures and test old-code/new-schema compatibility. Build the Docker image and run the live API/Celery smoke check against disposable seeded services. Pin third-party Actions to commit SHAs; PR jobs receive no production credentials.

**Merge to main:** rerun required checks against the actual merge commit, build and publish an immutable image tagged by commit SHA, and promote that exact digest through staging. A deployment job serializes migration execution, inspects the migration plan, applies only backward-compatible expansion migrations, then rolls out staging and runs smoke checks. Long backfills and later constraint/removal steps are separate resumable release operations, not hidden application-startup work. Never run demo seeding in production.

**Production gate:** require green staging checks, reviewed migration/rollback notes, and approval through a protected GitHub production environment. Use scoped, short-lived deployment credentials where supported. Allow one production release at a time. Run expansion migrations once while old instances remain live, then canary and roll the four instances with readiness checks and connection draining. Monitor errors, latency, database waits, and Celery backlog; stop promotion when agreed baselines are breached. Keep worker task payloads compatible during mixed-version operation, with only one Beat scheduler.

**Rollback:** redeploy the previous compatible image digest and retain the expanded schema/data. Do not automatically reverse migrations. Once constraints tighten or columns disappear, old binaries may be unsafe; honor the established compatibility floor and prefer a forward repair. Restoring a backup is an incident recovery action with possible intervening-write loss, not a routine rollback. [GitHub deployment controls](https://docs.github.com/en/actions/how-tos/deploy/configure-and-manage-deployments/control-deployments).
